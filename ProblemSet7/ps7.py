...
class WordTrigger(Trigger):
        def __init__(self,word):
                super(WordTrigger,self)
                self.word_ = word.lower()

        def isWordIn(self,text):
                for c in string.punctuation:
                        text = text.replace(c," ")
                return self.word_ in text.split(" ")
...
class NotTrigger(Trigger):
        def __init__(self,trigger):
                super(NotTrigger,self)
                self.trigger_ = trigger

	def evaluate(self,story):
		return not self.trigger_.evaluate(story)

# TODO: AndTrigger
class AndTrigger(Trigger):
        def __init__(self,trigger1,trigger2):
                super(AndTrigger,self)
                self.trigger1_ = trigger1
		self.trigger2_ = trigger2

	def evaluate(self,story):
		return  self.trigger1_.evaluate(story) and self.trigger2_.evaluate(story)


# TODO: OrTrigger
class OrTrigger(Trigger):
        def __init__(self,trigger1,trigger2):
                super(OrTrigger,self)
                self.trigger1_ = trigger1
		self.trigger2_ = trigger2

	def evaluate(self,story):
		return  self.trigger1_.evaluate(story) or self.trigger2_.evaluate(story)
...
class PhraseTrigger(Trigger):
        def __init__(self,phrase):
                super(PhraseTrigger,self)
		self.phrase_ = phrase

	def evaluate(self,story):
		return 	self.phrase_ in story.getTitle() or self.phrase_ in story.getSubject() or self.phrase_ in story.getSummary() or self.phrase_ in story.getGuid()
