...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        ## YOUR CODE HERE ####
	def get_ctype(parents_a,parents_b):
               	da = -1
               	for pa in parents_a:
                        da = da + 1
                        db = -1
        ...
        member_a = self.names_to_nodes[a]
	member_b = self.names_to_nodes[b]
	
        parents_of_a = []
        parents_of_b = []
        p = member_a.get_parent()
        while(p != None):
               	parents_of_a.append(p)
               	p = p.get_parent()
        p = member_b.get_parent()
        while(p != None):
	...
	c_type = -1      
        if   a == b:
		c_type = -1
	elif member_a.is_child(member_b) or member_b.is_child(member_a) :
		c_type = -1
	else:
      		c_type = get_ctype(parents_of_a,parents_of_b) 
	degree = abs(len(parents_of_a) - len(parents_of_b)) 
...
t, r = f.cousin("a", "d")
print "'a' is a", words[t], "cousin", r, "removed from 'd'"

t, r = f.cousin("d", "a")
print "'d' is a", words[t], "cousin", r, "removed from 'a'"

t, r = f.cousin("a", "o")
print "'a' is a", words[t], "cousin", r, "removed from 'o'"
